...
// Note: Password reset confirmation emails are now sent via the email queue system
// (see /api/auth/reset-password POST handler) for robust retry when GikpsMail is down.
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Send confirmation email via queue (non-blocking — queues if GikpsMail is down)
    try {
    ...
    <body style="margin:0;padding:0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;background-color:#f3f4f6;">
        ...
        <tr><td align="center">
          <table role="presentation" width="100%" max-width="500px" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 6px -1px rgba(0,0,0,0.1);">
            ...
            <tr><td style="padding:40px 30px;">
              ...
              <p style="margin:0 0 24px;font-size:16px;line-height:1.6;color:#6b7280;">Your password has been successfully reset. You can now log in to your CrimeReport account with your new password.</p>
              <table role="presentation" cellpadding="0" cellspacing="0" style="margin:0 auto;">
                <tr><td align="center">
                  <a href="${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/login" style="display:inline-block;padding:14px 32px;background-color:#dc2626;color:#ffffff;text-decoration:none;border-radius:8px;font-size:16px;font-weight:600;">Sign In to CrimeReport</a>
                </td></tr>
              </table>
              <div style="margin-top:24px;padding:16px;background-color:#fee2e2;border-radius:8px;font-size:13px;color:#991b1b;">⚠️ If you did NOT request this password reset, please contact support immediately. Your account may be compromised.</div>
            </td></tr>
            <tr><td style="background-color:#f9fafb;padding:24px 30px;text-align:center;border-top:1px solid #e5e7eb;">
              <p style="margin:0;font-size:12px;color:#9ca3af;">&copy; ${new Date().getFullYear()} CrimeReport System. All rights reserved.</p>
            </td></tr>
          </table>
        </td></tr>
      </table>
      ...
      console.log(`[ResetPassword] ✅ Confirmation email queued for ${userEmail}`);
    } catch (emailError) {
